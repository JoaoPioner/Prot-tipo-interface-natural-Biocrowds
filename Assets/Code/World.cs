...
namespace Biocrowds.Core
{
    public class World : MonoBehaviour
    {
        ...
        public List<Agent> group1 = new List<Agent>();
        public List<Agent> group2 = new List<Agent>();
        public List<Agent> group3 = new List<Agent>();
        public List<Agent> group4 = new List<Agent>();
        public List<Agent> group5 = new List<Agent>();
        ...
        internal void setInLoopFlagFalse(List<Agent> group)
        {
        ...
        public void Agents_Run(List<Agent> group)
        {
            foreach (Agent agent in group) {
        ...
        internal void DivideAndSetGroup(List<Agent> Group, int groupIndex)
        {
            Agent agent;
            for (int i = 0; i < Group.Count/2; i++)
            {
                agent = Group[0];
                agent.group = groupIndex;
                agent.meuTexto.text = "" + (groupIndex + 1);
                Group.Remove(agent);
                switch (groupIndex)
                {
                    case 0:
        ...
        public void setGroup(List<Agent> Group, int groupIndex)
        {
            Agent agent = Group[Group.Count - 1];
            agent.group = groupIndex;
            agent.meuTexto.text = "" + (groupIndex + 1);
            Group.Remove(agent);
            switch (groupIndex) {
            ...
            foreach (SpawnArea _area in spawnAreas)
            {
                for (int i = 0; i < _area.initialNumberOfAgents; i ++)
                {
                    if (_agents.Count < MAX_AGENTS)
                        SpawnNewAgentInArea(_area, true, selectedGroup);
                    yield return null;
                }
            }
        }
                ...
                if (_area.CycleReady)
                {
                    for (int i = 0; i < _area.quantitySpawnedEachCycle; i++)
                    {
                        if (_agents.Count < MAX_AGENTS)
                        {
                            //if (selectedGroup == 2)
                                //selectedGroup = 0;
                            SpawnNewAgentInArea(_area, false, selectedGroup);
                            //selectedGroup++;
                        }
                    }
                }
        ...
        private void SpawnNewAgentInArea(SpawnArea _area, bool _isInitialSpawn, int group)
        {
            ...
            if (_isInitialSpawn)
            {
                ...
                newAgent.goalsList.Add(_area.initialAgentsGoalList[0]);
                newAgent.Goal = newAgent.goalsList[0];  //agent goal
            ...
            {
                newAgent.goalsList = new List<GameObject>();
                newAgent.goalsList.Add(_area.repeatingGoalList[0]);
                newAgent.Goal = newAgent.goalsList[0];  //agent goal
        ...
        public void setMaxPop(float newMax)
        {
            MAX_AGENTS = newMax;
        }

        public void setSelectedGroup(int newGroup)
        {
        ...
        public void removeMaxPop()
        {
            _agentsToRemove.Add(_agents[_agents.Count - 1]);
            if (!(MAX_AGENTS < 1)) {
                MAX_AGENTS--;
            }
        ...
        public void playAnim(bool flag) {
            foreach (Agent agent in _agents) {
                agent.GetComponent<Animator>().SetTrigger("RunFlag");
            }
        }

        public void changeObjective(GameObject goal, List<Agent> Group) {
            List<GameObject> goalAsLst = new List<GameObject>();
            goalAsLst.Add(goal);
            foreach (Agent agent in Group) {
                agent.inLoop = false;
                agent.setGoalIndex(0);
                agent.goalsList = goalAsLst;                
            }
        }

        public void inLoopChangeObjective(List<GameObject> goals, List<Agent> Group)
        {
                foreach (Agent agent in Group)
                {
                    agent.inLoop = true;
                    agent.goalsList = goals;
                }
        }

        public void printGroupCounts()
        {
            Debug.Log("Group 1: " + group1.Count);
            foreach (Agent agent in group1) {
                agent.printAgentId();
            }
            Debug.Log("Group 2: " + group2.Count);
            foreach (Agent agent in group2)
            {
                agent.printAgentId();
            }
            Debug.Log("Group 3: " + group3.Count);
            foreach (Agent agent in group3)
            {
                agent.printAgentId();
            }
        }
